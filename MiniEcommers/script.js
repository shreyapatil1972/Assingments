    ...
    filteredProducts.map((product) => `<div class="col-sm-6">
        <div class="card">
          <div class="card-body">
            ...
            <p class="card-text">category: <span class='text'>${product.category}</span></p>
            <p class="card-text">Price: <span class='text'>${product.price}</span></p>
            <p class="card-text">stock: <span class='text'>${product.stock}</span></p>
            <button class="btn btn-primary" onclick='deletetask(${product.id})'>Delete</button>
            <button class="btn btn-primary" onclick='editProd(${product.id})'>Edit</button>
          </div>
        </div>
      </div>
  `).join('')
...
function updateProduct(ID){
  ...
  newStock = document.getElementById('upfatedStock').value
